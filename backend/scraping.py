...
def scrape_chaturbate_data(url, progress_callback=None):
    ...
    Returns:
        dict: Containing streamer_username and chaturbate_m3u8_url.
    """
    try:
        if progress_callback:
            progress_callback(10, "Fetching Chaturbate page")

        # Fetch the m3u8 URL using Selenium Wire and assign to a distinct variable
        ...
        result = {
            ...
            "chaturbate_m3u8_url": chaturbate_m3u8_url,  # Distinct key for Chaturbate
        }
...
def scrape_stripchat_data(url, progress_callback=None):
    ...
    Returns:
        dict: Containing streamer_username and stripchat_m3u8_url.
    """
    try:
        if progress_callback:
        ...
        result = {
            "streamer_username": streamer_username,
            "stripchat_m3u8_url": stripchat_m3u8_url,  # Distinct key for Stripchat
        }
        logging.info("Scraped details: %s", result)
